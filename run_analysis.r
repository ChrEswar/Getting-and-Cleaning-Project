## This script is created for the getting and cleaning data project question 1

getwd()
##################################################################################
# 1.  merging the training and test data files 
...
dtATrain <- readtable("train/Y_train.txt") ## training labels set
dtATest <- readtable("test/Y_test.txt")  ## test labels set

dtTrain <- readtable("train/X_train.txt")   ## training data set
dtTest <- readtable("test/X_test.txt")  ## test data set

# Merge training and test datasets::
dtSubject <- rbind(dtTrain, dtTest) ## contains the  training & test data set
dtActivity <- rbind(dtATrain, dtATest)  ## contains the Label for training & test set.
...
### Merged training and  test data test
##write.table(dt1,"fulldata.txt",row.name=FALSE)
